<template>
  <Listbox v-model="selectedUnit" v-slot="{ isOpen }" class="relative">
    <ListboxLabel class="sr-only">Select a Unit:</ListboxLabel>
    <label
      for="choose_unit"
    ...
    <ListboxButton
      v-slot="{ isFocused }"
      class="w-48 mt-1 focus:outline-none"
      id="choose_unit"
    >
      <span
        :class="
      ...
      >
        <span v-if="selectedUnit" class="text-gray-900">
          {{ selectedUnit }}</span
        >
    ...
    >
      <ListboxList
        v-show="isOpen"
        class="absolute z-50 w-48 mt-1 overflow-y-auto rounded-md shadow-lg max-h-56 focus:outline-none"
      >
        <ListboxOption
          v-for="unit in units"
          :key="unit"
